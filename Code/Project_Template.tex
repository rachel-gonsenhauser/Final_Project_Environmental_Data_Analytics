...
were chosen given their differences in geology and socioeconomic makeup.
...
\begin{quote}
***EDIT THIS LATER TO PUUT EMPHASIS ON ARSENIC AND PFAS AND EXPLAIN
WHY!!!!
\end{quote}
...
\begin{verbatim}
## Warning: Removed 49146 rows containing non-finite values (stat_bin).
\end{verbatim}
...
concenntratinos in North Carolina over this period of time remain safely
below the MCL standard. Arsenic concentrationns, on the other hand,
...
Additionally, there is no discernible relationship between arsenic
concentration and MHI (Figure 9).
\end{quote}
...
(Multiple linear regression; df=3 annd 242, F=1.664, p=0.1753). A large
range of arsenic cooncentratinos is experienced both across different
population sizes served by CWSs annd across MHI levels (Figure 10).
\end{quote}
...
\begin{quote}
Multiple linear regression results discussed previously indicated that
trihalomethane concentrations significantly predict arsenic
concentrations in North Carolina but not in Massachusetts (Figure 11).
\end{quote}
