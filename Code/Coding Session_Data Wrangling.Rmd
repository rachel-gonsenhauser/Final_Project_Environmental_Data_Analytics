...
colnames(Arsenic[8]) <-c("ArsenicMCL")
colnames(Arsenic)
...
colnames(Trihalomethane[8]) <-c("TrihalomethaneMCL")

colnames(Uranium)
head(Uranium)
summary(Uranium)
colnames(Uranium)[6] <-c("Uranium_ugL")
colnames(Uranium[8]) <-c("UraniumMCL")
```
...
Water.Quality.And.Income.Processed <- 
  ...
  select(stateFIPS:Arsenic_ugL, PWS.ID:Population.Served, MHI, PFAS_ppt, TTHM_ugL, Uranium_ugL)
